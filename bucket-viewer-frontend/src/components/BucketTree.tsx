...
const BucketTree = ({
    ...
    if (keyType === "delete") {
      api
        ...
        .catch(error => {
          console.error(error);
          messageApi.error(
            `Error during the delete, the file has not been deleted`
          );
        });
    } else if (keyType === "download") {
      api
        .get("/bucket/presign_url", {
          params: {
            keyFile: keyFile,
          },
        })
        .then(res => {
          const presign_url = res.data;
          window.open(presign_url, "_blank");
          messageApi.success("Download started");
        })
        .catch(error => {
          console.error("Download error: ", error);
          messageApi.error(`Failed to download: ${error}`);
        });
    }
  };
  const items: MenuProps["items"] = [
    {
      key: "download",
      label: <Text color="default"> Download </Text>,
      icon: <DownloadOutlined />,
      disabled: false,
    },
    {
      key: "delete",
      label: <Text color="default"> Delete </Text>,
      icon: <DeleteOutlined />,
      disabled: false,
    },
  ];
  ...
  const treeData: TreeDataNode[] = useMemo(() => {
    const prefixes =
      bucketData?.commonPrefixes?.map(prefix => {
        return {
          title: prefix.Prefix,
          key: prefix.Prefix,
          expandAction: true,
    ...
    const contents =
      bucketData?.contents?.map(content => {
        return {
          title: (
            <Dropdown
              menu={{
